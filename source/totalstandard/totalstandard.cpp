...
//天平采集串口 上位机直接采集
void TotalStandardDlg::initBalanceCom()
{
...
** 计算流速
*/
void TotalStandardDlg::slotFreshFlowRate()
{
...
int TotalStandardDlg::isWaterOutValveOpen()
{
	return true;
}
...
** 读取热表
*/
int TotalStandardDlg::readMeter()
{
...
//设置热量表进入检定状态
int TotalStandardDlg::setMeterVerifyStatus()
{
	for (int i=0; i<m_maxMeterNum; i++)
	{
...
//打开大流量点阀门
int TotalStandardDlg::openBigFlowValve()
{
	openValve(m_portsetinfo.bigNo);
	return true;
}
...
//关闭大流量点阀门
int TotalStandardDlg::closeBigFlowValve()
{
	closeValve(m_portsetinfo.bigNo);
	return true;
}
...
//响应处理天平质量的变化
int TotalStandardDlg::judgeBalanceInitValue(float v)
{
...
int TotalStandardDlg::judgeBalanceAndCalcAvgTemper(float targetV)
{
	int second;
...
//点击"下一步"按钮
void TotalStandardDlg::on_btnNext_clicked()
{
	if (m_nowOrder >= m_flowPointNum)
	{
...
** 连续检定时：满足总检定用量
*/
bool TotalStandardDlg::judgeBalanceCapacity()
{
	bool ret = false;
...
** 不连续检定时：满足单次检定用量
*/
int TotalStandardDlg::judgeBalanceCapacitySingle(int order)
{
	return true;
}
...
** 自动读取表流量成功 显示表流量
*/
void TotalStandardDlg::slotSetMeterFlow(const QString& comName, const float& flow)
{
	int meterPos = m_readComConfig->getMeterPosByComName(comName);
	if (meterPos < 1)
	{
...
void TotalStandardDlg::on_btnWaterOut_clicked() //出水阀
{
...
//请求读表（广播地址读表）
void TotalStandardDlg::on_btnReadMeter_clicked()
{
...
** 输入参数：
	row:行号，由row可以知道当前热表对应的串口、表号、误差等等
*/
void TotalStandardDlg::slotAdjustError(const int &row)
{
	qDebug()<<"adj row ="<<row;
}
