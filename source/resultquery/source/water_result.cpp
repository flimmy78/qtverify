...
WaterResultDlg::WaterResultDlg(QWidget *parent, Qt::WFlags flags)
...
//查询检定结果
void WaterResultDlg::on_btnQuery_clicked()
{
	getCondition();
	queryData();
...
void WaterResultDlg::getCondition()
{
	m_conStr.clear();
...
void WaterResultDlg::queryData()
{
	//model->setEditStrategy(QSqlTableModel::OnFieldChange); //属性变化时写入数据库
	model->setTable("T_Water_Verify_Record");
	...
	model->setHeaderData(6, Qt::Horizontal, QObject::tr("MeterV0"));//水表初值
	model->setHeaderData(7, Qt::Horizontal, QObject::tr("MeterV1"));//水表终值
	...
	model->setHeaderData(35, Qt::Horizontal, QObject::tr("WaterPress"));//水压
	...
	ui.tableView->hideColumn(40);
	ui.tableView->hideColumn(41);
}

void WaterResultDlg::on_btnStop_clicked()
{
	m_count = 1;
	qDebug()<<"on_btnStop_clicked, m_count ="<<m_count;
}

void WaterResultDlg::on_btnExport_clicked()
{
	if (NULL==model)
	{
	...
	if (!file.isEmpty())
	{
		try
		{
			...
			QString iniFileName = ui.cmbMethod->currentIndex() ? "rptconfig_water_std.ini":"rptconfig_water_mass.ini";
...
void WaterResultDlg::on_btnExit_clicked()
{
	this->close();
}

void WaterResultDlg::on_btnImport_clicked()
{
