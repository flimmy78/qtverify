...
//热量表通讯串口
void StdMtrCoeCorrect::initMeterCom()
{
	if (m_meterObj)
	{
		delete []m_meterObj;
...
//检测串口、端口设置是否正确
int StdMtrCoeCorrect::isComAndPortNormal()
{
	return true;
}
...
//初始化表格控件
void StdMtrCoeCorrect::initTableWidget()
{
	if (m_maxMeterNum <= 0)
	{
		return;
...
//显示当前关键参数设置信息
void StdMtrCoeCorrect::showNowKeyParaConfig()
{
	if (NULL == m_nowParams)
	{
		return;
...
int StdMtrCoeCorrect::isDataCollectNormal()
{
	return true;
}
...
** 读取所有热表流量系数
*/
int StdMtrCoeCorrect::readAllMeterFlowCoe()
{
...
//设置所有热量表进入检定状态
int StdMtrCoeCorrect::setAllMeterVerifyStatus()
{
...
** 点击"排气"按钮
*/
void StdMtrCoeCorrect::on_btnExhaust_clicked()
{
...
//点击"重新计算"按钮
void StdMtrCoeCorrect::on_btnReCalc_clicked()
{
}
...
int StdMtrCoeCorrect::getValidMeterNum()
{
...
返回:被检表下标，从0开始
*/
int StdMtrCoeCorrect::isMeterPosValid(int meterPos)
{
	for (int i=0; i<m_validMeterNum; i++)
	{
...
** 计算所有被检表的误差
*/
int StdMtrCoeCorrect::calcAllMeterError()
{
	int ret = 1;
	for (int i=0; i<m_validMeterNum; i++)
	{
...
** 返回值：1-计算成功、读表数据全部成功
		   0-计算失败、读表数据有失败的
*/
int StdMtrCoeCorrect::calcVerifyResult()
{
	int ret = 0;
...
void StdMtrCoeCorrect::exportReport()
{
...
//自动读取表号成功 显示表号
void StdMtrCoeCorrect::slotSetMeterNumber(const QString& comName, const QString& meterNo)
{
...
** 自动读取表流量成功 显示表流量
*/
void StdMtrCoeCorrect::slotSetMeterFlow(const QString& comName, const QString& flow)
{
	int meterPos = m_readComConfig->getMeterPosByComName(comName);
	if (meterPos < 1)
	{
...
** 自动读取表流量成功 刷新大流量点系数
*/
void StdMtrCoeCorrect::slotFreshBigCoe(const QString& comName, const QString& bigCoe)
{
	int meterPos = m_readComConfig->getMeterPosByComName(comName);
	if (meterPos < 1)
	{
...
** 自动读取表流量成功 刷新中二流量点系数
*/
void StdMtrCoeCorrect::slotFreshMid2Coe(const QString& comName, const QString& mid2Coe)
{
	int meterPos = m_readComConfig->getMeterPosByComName(comName);
	if (meterPos < 1)
	{
...
** 自动读取表流量成功 刷新中一流量点系数
*/
void StdMtrCoeCorrect::slotFreshMid1Coe(const QString& comName, const QString& mid1Coe)
{
	int meterPos = m_readComConfig->getMeterPosByComName(comName);
	if (meterPos < 1)
	{
...
** 自动读取表流量成功 刷新小流量点系数
*/
void StdMtrCoeCorrect::slotFreshSmallCoe(const QString& comName, const QString& smallCoe)
{
	int meterPos = m_readComConfig->getMeterPosByComName(comName);
	if (meterPos < 1)
	{
...
//上一次的终值作为本次的初值
void StdMtrCoeCorrect::makeStartValueByLastEndValue()
{
// 	clearTableContents();
	for (int i=0; i<m_validMeterNum; i++)
	{
...
//请求读表号（所有表、广播地址读表）
void StdMtrCoeCorrect::on_btnAllReadNO_clicked()
{
...
//请求读表数据（所有表、广播地址读表）
void StdMtrCoeCorrect::on_btnAllReadData_clicked()
{
...
//设置检定状态（所有表）
void StdMtrCoeCorrect::on_btnAllVerifyStatus_clicked()
{
	for (int i=0; i<m_maxMeterNum; i++)
	{
		slotVerifyStatus(i);
...
//调整误差（所有表）
void StdMtrCoeCorrect::on_btnAllAdjError_clicked()
{
	for (int i=0; i<m_maxMeterNum; i++)
	{
		slotAdjustError(i);
...
//修改表号（所有表）
void StdMtrCoeCorrect::on_btnAllModifyNO_clicked()
{
	for (int i=0; i<m_maxMeterNum; i++)
	{
		slotModifyMeterNO(i);
...
** 输入参数：
   ...
   注意：表号为14位
*/
void StdMtrCoeCorrect::slotModifyMeterNO(const int &row)
{
...
** 输入参数：
	row:行号，由row可以知道当前热表对应的串口、表号、误差等等
*/
void StdMtrCoeCorrect::slotAdjustError(const int &row)
{
...
row:行号，由row可以知道当前热表对应的串口、表号、误差等等
*/
void StdMtrCoeCorrect::slotReadData(const int &row)
{
...
row:行号，由row可以知道当前热表对应的串口、表号、误差等等
*/
void StdMtrCoeCorrect::slotVerifyStatus(const int &row)
{
...
row:行号，由row可以知道当前热表对应的串口、表号、误差等等
*/
void StdMtrCoeCorrect::slotReadNO(const int &row)
{
...
** 保存起始表号
*/
void StdMtrCoeCorrect::saveStartMeterNO()
{
