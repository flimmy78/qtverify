...
QExcel::QExcel(QString xlsFilePath, QObject *parent)
{
	...
	{
		qDebug()<<"file: " << xlsFilePath << "does not exist!";
		this->close();
	}
}
...
void QExcel::unmergeCells(const QString& cell)
{
	QAxObject *range = sheet->querySubObject("Range(const QString&)", cell);
	range->dynamicCall("UnMerge()");
}
...
void QExcel::deleteRow(int startRow, int endRow)
{
