...
select
  ...
  rec.F_MethodFlag,
  round(rec.F_MeterValue0,2) F_MeterValue0,
  round(rec.F_MeterValue1,2) F_MeterValue1,
  round((rec.F_MeterValue1 - rec.F_MeterValue0), 2) F_MeterDispValue,
  round(rec.F_BalWeight0, 3) F_BalWeight0,
  round(rec.F_BalWeight1, 3) F_BalWeight1,
  round((rec.F_BalWeight1 - rec.F_BalWeight0), 3) F_BalDisp,
  round(rec.F_StdMeterV0, 4) F_StdMeterV0,
  round(rec.F_StdMeterV1, 4) F_StdMeterV1,
  round((rec.F_StdMeterV1 - rec.F_StdMeterV0), 4) F_StdDisp,
  round(rec.F_PipeTemper, 2) F_PipeTemper,
  round(rec.F_Density, 5) F_Density,
  round(rec.F_StandValue, 3) F_StandValue,
  round(rec.F_DispError, 3) F_DispError,
  round(rec.F_StdError, 3) F_StdError,
  rec.F_Grade,
...
select
	...
	round(rec.F_FlowPoint,3) F_FlowPoint,
	rec.F_MethodFlag,
	round(rec.F_MeterValue0,3) F_MeterValue0,
	round(rec.F_MeterValue1,3) F_MeterValue1,
	round((rec.F_MeterValue1-rec.F_MeterValue0), 3) F_MeterDispValue,
	round(rec.F_BalWeight0, 3) F_BalWeight0,
	round(rec.F_BalWeight1, 3) F_BalWeight1,
    round((rec.F_BalWeight1 - rec.F_BalWeight0), 3) F_BalDisp,
	round(rec.F_StdMeterV0, 4) F_StdMeterV0,
...
select
  ...
  round(rec.F_PipeTemper, 2) F_PipeTemper,
  round(rec.F_Density, 5) F_Density,
  round(rec.F_StandValue, 2) F_StandValue,
  round(rec.F_DispError, 3) F_DispError,
  round(rec.F_StdError, 3) F_StdError,
  rec.F_Grade,
  (select F_Desc from T_User_Def_Tab u where u.F_id = rec.F_VerifyPerson)  F_VerifyPerson,
